...
class AddIdCardScreen extends StatefulWidget {
  ...
  @override
  State<AddIdCardScreen> createState() => _AddIdCardScreenState();
}

class _AddIdCardScreenState extends State<AddIdCardScreen> {
  ...
  DateTime? extractExpirationDate(String text) {
    RegExp datePattern = RegExp(r'\d{2}\.\d{2}\.\d{2}-\d{2}\.\d{2}\.\d{4}');
    Match? match = datePattern.firstMatch(text);

    if (match != null) {
      String? valability = match[0];
      String? expirationDate =
          valability?.substring(valability.indexOf('-') + 1);
