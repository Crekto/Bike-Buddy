import 'dart:io';
import 'package:bike_buddy/screens/bikes/screens/documents/documents_shared_methods.dart';
...
class _AddInsuranceScreenState extends State<AddInsuranceScreen> {
                  ...
                  ElevatedButton(
                    onPressed: () async {
                      if (_formKey.currentState?.saveAndValidate() ?? false) {
                        ...
                        List<String> images = [];
                        var uploadedImages =
                            await _formKey.currentState?.fields['image']!.value;
                        ...
                        if (documents == null) {
                          ...
                          documents.insurance = MyDocument(
                              images,
                              _formKey
                                  .currentState?.fields['expiration']!.value);
                        }
                        ...
                        scheduleNotifications(
                            400,
                            _formKey.currentState?.fields['expiration']!.value,
                            "Insurance");
  ...
  DateTime? extractExpirationDate(String text) {
    final RegExp datePattern = RegExp(r'pana la:\s*(\d{2}\.\d{2}\.\d{4})');
    final Match? match =
        datePattern.firstMatch(removeDiacritics(text).toLowerCase());

    if (match != null) {
      debugPrint("DATA EXPIRARRIIIII1: ${match[0]}");
      String? expirationDate =
          match[0]?.replaceAll(RegExp(r'pana la:|\s*'), '');
      debugPrint("DATA EXPIRARRIIIII2: $expirationDate");
